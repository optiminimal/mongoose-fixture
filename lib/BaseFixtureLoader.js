...
module.exports = function(fixtureConfig, mongoose, rootCallback){
    ...
    var nestedFixtureData = require(fixtureConfig.dataFixturePath+fixtureDataKey);

    var fixtureData = [];
    for( var p in nestedFixtureData ) {
        for( var q in nestedFixtureData[ p ] ) {
            fixtureData.push( nestedFixtureData[ p ][ q ] );
            console.info('adding item: %s', q);
        }
    }
    ...
    async.forEach(fixtureData, 
        ...
        function(dataDoc, returnLoaderCallback){
            // console.info(dataDoc);
            // use the loaded fixtureModel's mongoose static .create method
            fixtureModel.create(dataDoc, function(err, aItem){
                if(err){ 
        ...
        function(err){
            ...
            console.log('Completed %s fixture loading\n',fixtureDataKey);
            rootCallback();
        }
    ); // end async-forEach
};
